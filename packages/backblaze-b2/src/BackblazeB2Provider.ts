...
class BackblazeB2Provider extends StorageProvider {
     ...
     * 2. In the situation when `data` is a stream and the length can't be known beforehand, if the data is longer than `B2Upload.chunkSize` (default: 8MB; minimum: 5MB) the method will use B2's [large files APIs](https://www.backblaze.com/b2/docs/large_files.html). With those, it's possible to chunk the file into many chunks and upload them separately, thus it's not necessary to load the entire strema in memory. However, this way of uploading files requires many more network calls, and could be significantly slower. Maximum size is 200GB (using 8MB chunks - configurable with the `B2Upload.chunkSize` property).
     * 
